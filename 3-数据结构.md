...
struct chairman_tree {
    ...
    void update(int x, int add, int& rt, int& lst, int l = 1,int r = all)
    {
        rt = ++tot;
        tr[rt].cnt = tr[lst].cnt + add;
        if (l == r) return;
        int mid = (l + r) >> 1;
        if (x <= mid) {
            tr[rt].rs = tr[lst].rs;
            update(x, add, tr[rt].ls, tr[lst].ls, l, mid);
        } else {
            tr[rt].ls = tr[lst].ls;
            update(x, add, tr[rt].rs, tr[lst].rs, mid + 1, r);
        }
    }

    int query(int k, vector<int> &rt, vector<int> &lst, int l = 1,int r = all)
    {
        if (l == r) return l;
        int mid = (l + r) >> 1;
        int s = 0;
        for (auto& o: rt) s += tr[tr[o].ls].cnt;
        for (auto& o: lst) s -= tr[tr[o].ls].cnt;
        if (s < k) {
            for (auto& o: rt) o = tr[o].rs;
            for (auto& o: lst) o = tr[o].rs;
            return query(k-s, rt, lst, mid + 1, r);
        }
        else {
            for (auto& o: rt) o = tr[o].ls;
            for (auto& o: lst) o = tr[o].ls;
            return query(k, rt, lst, l, mid);
        }
    }
}cmt;
...
void upd(int p,int x,int add) {
    for (; p <= n; p += lowbit(p)) {
        int nrt;
        cmt.update(x,add,nrt,cmt.root[p]);
        cmt.root[p] = nrt;
    } 
}
...
int qry(int l,int r,int k) {
    vector<int> rt,lst;
    for (l = l-1; l; l -= lowbit(l)) lst.push_back(cmt.root[l]);
    for (; r; r -= lowbit(r)) rt.push_back(cmt.root[r]);
    return cmt.query(k,rt,lst);
}

int idx(int x) { return lower_bound(c + 1, c + 1 + all, x) - c; }
```
...
struct splay_tree
{
    ...
    void update(int x) {
        nd[x].sz = nd[nd[x].son[0]].sz + nd[nd[x].son[1]].sz + nd[x].cnt;
    }
    ...
    void rotate(int x) {
        ...
        int z = nd[y].fa;
        int k = (nd[y].son[1] == x);
        nd[z].son[nd[z].son[1]==y] = x;
        nd[x].fa = z;
        nd[y].son[k] = nd[x].son[k^1];
        nd[nd[x].son[k^1]].fa = y;
        nd[x].son[k^1] = y;
        nd[y].fa = x;
        update(y);update(x);
    }
    ...
    void splay(int x,int goal) {
        while (nd[x].fa != goal) {
            ...
            if (z != goal) {
                (nd[z].son[0]==y)^(nd[y].son[0]==x)?rotate(x):rotate(y);
            }
            rotate(x);
        }
        if (goal == 0) root = x;
    ...
    void Find(int x) {
        int u = root;
        if (u == 0) return;
        while (nd[u].son[x>nd[u].val] && nd[u].val != x) {
            u = nd[u].son[x>nd[u].val];
        }
        splay(u,0);
    }
    ...
    void Insert(int x) {
        ...
        if (u) {
            ...
            u = ++tot;
            nd[u].son[0] = nd[u].son[1] = 0;
            nd[u].val = x;
            nd[u].cnt = 1;
            nd[u].fa = fa;
            nd[u].sz = 1;
            if (fa) nd[fa].son[x>nd[fa].val] = u;
        }
        splay(u,0);
    }

    //f: lst 0, nxt 1 
    int Next(int x,bool f) {
        Find(x);
        int u = root;
        if (nd[u].val > x && f) return u;
        if (nd[u].val < x && !f) return u;
        u = nd[u].son[f];
    ...
    int Kth(int k) {
        int u = root;
        if (nd[u].sz < k) return 0;
        while (true) {
            int y = nd[u].son[0];
            if (k > nd[y].sz + nd[u].cnt) {
                k -= (nd[y].sz + nd[u].cnt);
                u = nd[u].son[1];
            }
            else if (nd[y].sz >= k) u = y;
            else return nd[u].val;
        } 
    }
}spt;
